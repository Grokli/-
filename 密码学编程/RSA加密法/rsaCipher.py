...
def encryptAndWriteToFile(messageFilename, keyFilename, message, blockSize=DEFAULT_BLOCK_SIZE):
    ...
    if keySize < blockSize * 8:
        sys.exit('ERROR:Block size is %s bits and key size is %s bits.The RSA cipher requires the block size'
                 'to be equal to or less than the key size.Either decrease the block size or use different keys.' % (blockSize * 8, keySize))
...
def readFromFileAndDecrypt(messageFilename, keyFilename):
    ...
    if keySize < blockSize * 8:
        sys.exit('ERROR: Block size is %s bits and key size is %s bits. The RSA cipher requires '
                 'the block size to be equal to or greater than the key size. '
