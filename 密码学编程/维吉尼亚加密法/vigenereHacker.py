...
def main():
    ...
    umqcifm. Mtoej bts raj pq kjrcmp oo tzm Zooigvmz Khqauqvl Dincmalwdm, rhwzq 
    vz cjmmhzd gvq ca tzm rwmsl lqgdgfa rcm a kbafzd-hzaumae kaakulmd, hce SKQ. 
    Wi 1948 Tmzubb jgqzsy Msf Zsrmsv'e Qjmhcfwig Dincmalwdm vt Eizqcekbqf Pnadqfnilg, 
    ...
    fgtxcrifo mb Dnlmdbzt uiydviyv, Nfdtaat Dmiem Ywiikbqf Bojlab Wrgez avdw iz cafakuog 
    pmjxwx ahwxcby gv nscadn at ohw Jdwoikp scqejvysit xwd "hce sxboglavs kvy zm ion 
    tjmmhzd." Sa at Haq 2012 i bfdvsbq azmtmd'g widt ion bwnafz tzm Tcpsw wr Zjrva ivdcz 
    eaigd yzmbo Tmzubb a kbmhptgzk dvrvwz wa efiohzd."""
...
def getMostCommonFactors(seqFactors):
    ...
    for seq in seqFactors:
        ...
        for factor in factorList:
            if factor not in factorsCounts:
                factorsCounts[factor] = 0
            factorsCounts[factor] += 1
    factorsByCount = []
    for factor in factorsCounts:
        if factor <= MAX_KEY_LENGTH:
            factorsByCount.append((factor, factorsCounts[factor]))
...
def attemptHackWithKeyLength(ciphertext, mostLikelyKeyLength):
    ...
    for indexs in itertools.product(range(NUM_MOST_FREQ_LETTERS), repeat=mostLikelyKeyLength):
        possibleKey = ''
        for i in range(mostLikelyKeyLength):
            possibleKey += allFreqScores[i][indexs[i]][0]
        if not SILENT_MODE:
